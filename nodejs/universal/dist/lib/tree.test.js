...
describe('Path Tree', () => {
    ...
    const m3Component1Service1 = {
        path: 'home/module3-i/component1-i/service1-t',
    };
    const m3Component1Service2 = {
    ...
    it('populates path tree from generator', () => {
        ...
        expect(m2Component1Service1Node?.ancestors.map(n => [n.qualifiedPath, n.level])).toEqual([
    ...
    it('populates path tree from array', () => {
        const tree = t.pathTree([home, root2, module1, m1Component1, module2, m2Component1, m2Component1Service1], ts => ts.terminal.path.split('/'), node => ({
            ...node,
            intermediary: { friendlyName: h.humanFriendlyPhrase(node.qualifiedPath) },
        }));
        const nodes = t.pathTreeNodesList(tree, () => true);
