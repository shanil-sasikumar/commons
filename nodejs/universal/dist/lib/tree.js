...
export const qualifiedPathPreparer = (pathSep = '/') => {
    return (unit, parent) => parent?.qualifiedPath ? parent?.qualifiedPath + pathSep + unit : unit;
};
/**
...
export function selectTreeNode(node, query, options) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (activePart == '..') {
            pathIndex--;
            result = pathIndex >= 0 && pathIndex < path.length ? path[pathIndex] : undefined;
        }
        else {
...
export function populatePathTreeNodes(nodes, populate, inspector, level = 0, maxLevel) {
    ...
    for (const node of filtered) {
        if (typeof maxLevel !== 'number' || (level <= maxLevel && node.children.length > 0)) {
...
export function pathTree(entries, unitsSupplier, intermediarySupplier, options) {
    const tree = options?.tree ?? { children: [] };
