...
import * as mod from './memoize-foreign-content.js';
describe('custom foreignReadableHtmlMemoizer', () => {
    ...
    it('retrieves and memoizes readable content', async () => {
        const memoizableReadableContent = memoizableReadable('https://jojozhuang.github.io/tutorial/mermaid-cheat-sheet/');
...
describe('typical memoizableForeignReadable', () => {
    it('retrieves and memoizes unsanitized HTML as DOM', async () => {
        ...
        const mfh = memoizableForeignHTMLWithOptions('https://jojozhuang.github.io/tutorial/mermaid-cheat-sheet/', {
            extractHTML: dom => dom.window.document.querySelectorAll('div.post')[0]?.outerHTML ?? 'NOT FOUND',
        });
    ...
    it('retrieves and memoizes readable content', async () => {
        const memoizableForeignReadableWithOptions = await mod.memoizableForeignReadable({
            // TODO: Maybe "isInDevelopment" should be passed as an argument
            isInDevelopment: false,
        });
        const mrc = await memoizableForeignReadableWithOptions('https://jojozhuang.github.io/tutorial/mermaid-cheat-sheet/')();
        expect(mrc?.content).toBeDefined();
        expect(mrc?.textContent).toBeDefined();
    });
});
