...
export const memoizableForeignHTML = async (options: { readonly isInDevelopment: boolean }) =>
...
export const memoizableForeignReadable = async (options: { readonly isInDevelopment: boolean }) =>
...
export async function memoizableForeignContent(
  ...
  if (!foreignContent.content || foreignContent.content.readable) {
    ...
    return (
      (await memoizableForeignReadableWithOptions(foreignContent.url)())?.content ??
