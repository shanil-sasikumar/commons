...
import * as tp from './tree-pathway.js';
  ...
  it('populates path tree pathway from generator', () => {
    const tree = t.pathTree<Terminal, Intermediary>(
      function* () {
        ...
        yield { path: 'home/module3-i/component1-i/service1-t' };
        yield { path: 'home/module3-i/component1-i/service2-t' };
      },
      ts => ts.terminal.path.split('/'),
      node => ({
        ...node,
        intermediary: { friendlyName: h.humanFriendlyPhrase(node.qualifiedPath) },
      })
    );

    const bcUnit: tp.TreePathwayUnitSupplier<Terminal, Intermediary, BreadcrumbUnit> = node => ({
      label: node.unit,
    ...
    const breadcrumbs1 = prepareBreadcrumbs1.byNodeKey('home/module2/component1/service1');
    expect(breadcrumbs1).toStrictEqual([
      { label: 'home', slug: 'home' },
      { label: 'module2', slug: 'home/module2' },
      { label: 'component1', slug: 'home/module2/component1' },
      { label: 'service1', slug: 'home/module2/component1/service1' },
    ]);

    const prepareBreadcrumbs2 = tp.treePathwaysPreparer(tree, bcUnit, bcUnit, {
      refine: pathway => {
        pathway.unshift({ label: 'special-prefix', slug: 'prefix' });
        pathway.push({ label: 'special-suffix', slug: 'suffix' });
        return pathway;
      },
    });
    expect(prepareBreadcrumbs1).toBeDefined();
